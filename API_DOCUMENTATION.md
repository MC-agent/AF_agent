...
#### 참고
- user_id는 현재 로그인한 사용자의 ID로 자동 설정됩니다
...
### 채팅 히스토리 관리
- 모든 채팅 API는 인증이 필요합니다.
- 채팅은 사용자별로 자동으로 관리되며, 각 사용자는 자신의 채팅만 조회/수정/삭제할 수 있습니다.
- `GET /api/chats`를 호출하면 현재 로그인한 사용자의 모든 채팅 목록을 가져올 수 있습니다.
- 프론트엔드에서 localStorage에 현재 선택된 chat_id만 저장하면 됩니다.

### 새 채팅 플로우
1. 로그인 → JWT 토큰 획득
2. "새 채팅" 버튼 클릭 → `POST /api/chats` 호출 (토큰 필요)
3. 받은 `chat_id`를 현재 활성 채팅으로 설정
4. 첫 메시지 전송 → `POST /api/chats/{chat_id}/messages` (토큰 필요)
5. 첫 메시지 내용을 요약하여 → `PATCH /api/chats/{chat_id}/title` (토큰 필요)
6. 사이드바 목록은 `GET /api/chats`로 갱신 (토큰 필요)
