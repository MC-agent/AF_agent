...
supervisor = create_supervisor([restaurant_agent, room_check_agent],model=llm,prompt=(
    ...
    "숙소, 호텔, 게스트하우스, 리조트, 객실, 예약 가능 여부, 숙소 위치/상세 정보 요청은 room_check_agent에게 위임해. room_check_agent는 pgvector 숙소 검색과 예약/상세/위치 도구를 사용할 수 있어."
...
if __name__ == "__main__":
    result = run_supervisor("마포에 있는 중국집 추천해줘", thread_id="manual-test")
    print(result)
